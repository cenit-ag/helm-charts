...
- [5. Configuration Examples](#5-configuration-examples)
  - [5.1. One Volume Approach](#51-one-volume-approach)
  - [5.2. SM Server Persistence](#52-sm-server-persistence)
    - [5.2.1. Via H2 Database on a Persistent Volume](#521-via-h2-database-on-a-persistent-volume)
    - [5.2.2. Via External Database Services](#522-via-external-database-services)
  - [5.3. Dependency Injection (JDBC drivers, P8 API JARs, etc.)](#53-dependency-injection)
  - [5.4. Restricted Pod Permissions](#54-restricted-pod-permissions)
    - [5.4.1. Option A: RBAC Resources Created by the Chart](#541-option-a-rbac-resources-created-by-the-chart)
    - [5.4.2. Option B: RBAC Resources Created by the User](#542-option-b-rbac-resources-created-by-the-user)
    - [5.4.3. Example of a Minimum Policy](#543-example-of-a-minimum-policy)
  - [5.5. Provide Root Certificates to SM Server](#55-provide-root-certificates-to-sm-server)
    - [5.5.1. Replace cacerts of the JRE](#551-replace-cacerts-of-the-JRE)
    - [5.5.2. Provide Certificate as Secret](#552-provide-certificate-as-secret)
    - [5.5.3. Refer to Certificate Secret in Chart Configuration](#553-refer-to-certificate-secret-in-chart-configuration)
  - [5.6. TLS protocols and ciphers for web access](#56-tls-protocols-and-ciphers-for-web-access)
...
| `global.imagePullSecrets` | Optionally specify an array of imagePullSecrets for pulling images from a private container registry. Secrets must be manually created in the namespace. | `[{"name": "pull-secret-to-be-used"}]` |
...
| `global.chartHookDeletionPolicy` |  | `"before-hook-creation"` |
| `global.nameOverride` | Optional setting to overwrite the app's short name. | `""` |
| `global.fullnameOverride` | Optional setting  to overwrite the default fully qualified app name. | `""` |
| `jetty.tls.includeProtocols` | Optional list of allowed TLS protocol versions. Possible values are 'TLSv1.2' and 'TLSv1.3'. | `["TLSv1.3"]` |
| `jetty.tls.excludeProtocols` | Optional list of forbidden TLS protocol versions. Possible values are 'TLSv1.0', 'TLSv1.1', 'TLSv1.2', 'TLSv1.3'. | `["TLSv1.0", "TLSv1.1", "TLSv1.2"]` |
| `jetty.tls.includeCipherSuites` | Optional list of allowed cipher suites. Values must follow the pattern used by Jetty and the JVM; e.g. `TLS_AES_256_GCM_SHA374`. If empty, the default set of Jetty is used. | `""` |
| `jetty.tls.excludeCipherSuites` | Optional list of forbidden cipher suites. Values must follow the pattern used by Jetty and the JVM; e.g. `TLS_AES_256_GCM_SHA374`. If empty, the defulat set of Jetty is used. | `[".*RC4.*", ".*MD5.*"]` |
| `server.enabled` | Creates a StatefulSet for a SM Server. | `true` |
...
| `server.service.type` | The 'type' of IP used by the server pod.  | `"ClusterIP"` |
| `server.service.httpPort` | HTTP port of the server's web server.  | `8080` |
| `server.service.httpsPort` | HTTPS port of the server's web server.  | `8443` |
| `server.service.mqttPort` | Port used for the agent communication via MQTT.  | `1883` |
| `server.mqttLoadBalancer.enabled` | Used to connect to non-cluster agents.  | `false` |
| `server.mqttLoadBalancer.loadBalancerIP` | The IP is used by the load balancer. | `""` |
| `server.mqttLoadBalancer.ports` | Mapping of MQTT ports for the load balancer.  | `[{"name": "mqtt", "protocol": "TCP", "port": 1883, "targetPort": 1883}]` |
...
| `server.logLevel` | For supported Log4J log levels see  https://logging.apache.org/log4j/2.x/manual/customloglevels.html. Intensive logging (e.g. "DEBUG" or "TRACE") should only be used temporarily for trouble shooting. | `"ERROR"` |
| `server.terminationGracePeriodSeconds` | Waiting period before a terminating pod is forcefully ended. To avoid data loss, we recommend using a value greater or equal to 300 seconds. | `300` |
...
| `server.singleVolume.enabled` | If enabled the mount for the H2 database (see below under 'persistence'), the deploy-folder and the agent sync folder are all from one external volume that must provide the correct folder structure for the then needed sub-paths. | `false` |
| `server.singleVolume.existingClaim` | The identifier of the Persistent Volume Claim to be used for the volume mount. | `""` |
| `server.singleVolume.storageClass` |  | `""` |
| `server.singleVolume.accessModes` | Do not change this from its default if not told so explicitly. | `["ReadWriteMany"]` |
| `server.singleVolume.size` | At least 10Gi (8Gi for the database files) are needed if the server uses the H2 database.  | `"20Gi"` |
| `server.singleVolume.annotations` |  | `{}` |
| `server.singleVolume.volumePermissions.enabled` |  | `false` |
| `server.persistence.enabled` | Use a separate mount and underlying PVC for the H2 database files. | `false` |
...
| `server.persistence.size` | If the size is too low the H2 datbase can be damaged and dataloss will happen. | `"8Gi"` |
| `server.persistence.annotations` |  | `{}` |
| `server.persistence.volumePermissions.enabled` |  | `false` |
| `server.agentsSync.enabled` | Use a separate mount and underlying PVC for the agent sync folder and files. | `false` |
| `server.agentsSync.existingClaim` | If 'existingClaim' is defined, a PVC must be created manually before the volume can be bound. | `""` |
| `server.agentsSync.storageClass` |  | `""` |
| `server.agentsSync.accessModes` |  | `["ReadWriteOnce"]` |
| `server.agentsSync.size` | Size depends on the files used for synchronization. | `"8Gi"` |
| `server.agentsSync.annotations` |  | `{}` |
| `server.agentsSync.volumePermissions.enabled` |  | `false` |
| `server.externalDependenciesVolume.enabled` | Use a separate mount and underlying PVC for 3rd-party libs needed, e.g. JDBC-driver JARs or P8-API-JARs. | `false` |
| `server.externalDependenciesVolume.existingClaim` | If 'existingClaim' is defined, a PVC must be created manually before the volume will be bound. | `""` |
| `server.externalDependenciesVolume.storageClass` |  | `""` |
| `server.externalDependenciesVolume.accessModes` |  | `["ReadWriteOnce"]` |
| `server.externalDependenciesVolume.size` | Size depends on the files needed. | `"1Gi"` |
| `server.externalDependenciesVolume.annotations` |  | `{}` |
| `server.externalDependenciesVolume.volumePermissions.enabled` |  | `false` |
| `server.caCerts` | Configuration object for a Secret mounted as JVM trust store (`cacerts`).  This is the recommended way to use your own set of certificates. This setting is mutually exclusive with `server.certificates`. | `{}` |
| `server.caCerts.secretName` | Name of the Secret containing the `cacerts` file to mount into the server container. | `""` |
| `server.certificates` | List of certificates to import into the `cacerts` file via init container. Each item requires `secretName` and `secretKeys`. We recommend to use `server.caCerts`. This setting is mutually exclusive with `server.caCerts`. | `[]` |
| `server.paxWebSecurity.enabled` | Enable or disable the TLS protocol and cipher suite settings made in`jetty.tls` for the server's Jetty web server. | `false` |
| `server.extraInitContainers` | Normally not needed option to define extra init containers. It must be complete and working definitions of init containers. Use at your own riks. | `[]` |
| `server.extraEnv` | Optional list of environment variables. | `[]` |
...
| `server.externalConfigurationDatabase.jdbcUrl` | Depends on the database type and your environment. | `""` |
| `server.externalConfigurationDatabase.jdbcDriverClass` | Depends on the database type. | `""` |
| `server.externalConfigurationDatabase.jdbcSecret` | Secret to be used to access the external database. | `""` |
...
| `server.externalMonitoringDatabase.jdbcUrl` | Depends on the database type and your environment. | `""` |
| `server.externalMonitoringDatabase.jdbcDriverClass` | Depends on the database type. | `""` |
| `server.externalMonitoringDatabase.jdbcSecret` | Secret to be used to access the external database. | `""` |
| `agent.enabled` | Creates a Deployment for one SM Agent. Enabled by default. The minimum useful set for monitoring is one server and one agent. | `true` |
...
| `agent.serverHostname`| Server hostname used by the agent when `server.enabled=false`. Name service lookup must be able to resolve the IP and the network configuration must allow access to the server as well. | `""`                                                                                                                                                                                     |
...
| `agent.logLevel` | For supported Log4J log levels see: https://logging.apache.org/log4j/2.x/manual/customloglevels.html. Intensive logging (e.g. 'DEBUG' or 'TRACE') should be used only temporarily for trouble shooting. | `"ERROR"` |
| `agent.terminationGracePeriodSeconds` | Waiting period in seconds before a terminating pod is forcefully ended. To avoid dataloss, we recommend using a value greater or equal to 300 seconds. | `300` |
...
| `agent.livenessProbe.failureThreshold` | Should not be lower than 3. | `3` |
...
| `agent.caCerts.secretName` | Name of the Secret containing the `cacerts` file to mount into the agent container. | `""`                                                                                                                                                                                     |
| `agent.certificates` | List of certificates to import into the `cacerts` file via init container. Each item requires `secretName` and `secretKeys`. We recommend to use `server.caCerts` instead. This setting is mutually exclusive with `agent.caCerts`. | `[]` |
| `agent.paxWebSecurity.enabled` | Enable or disable the TLS protocol and cipher suite settings made in`jetty.tls` for the agent's Jetty web server. | `false` |
| `agent.extraInitContainers` | Normally not needed option to define extra init containers. It must be complete and working definitions of init containers. Use at your own riks. | `[]` |
| `agent.extraEnv` | Optional list of environment variables. | `[]` |
| `agent.singleVolume.enabled` | If enabled the mount for the deploy-folder is from one external volume that must provide the correct folder structure for the then needed sub-path. This allows to use one volume for this purpose for server and agent. | `false` |
| `agent.singleVolume.existingClaim` | The identifier of the Persistent Volume Claim to be used for the volume mount. | `""` |
| `agent.singleVolume.storageClass` |  | `""` |
| `agent.singleVolume.accessModes` | Do not change this from its default if not told so explicitly. | `["ReadWriteMany"]` |
| `agent.singleVolume.size` | The size must be kept in sync with the setting in 'server.singleVolume.size' if the same volume is used here.  | `"20Gi"` |
| `agent.singleVolume.annotations` |  | `{}` |
| `agent.externalDependenciesVolume.enabled` | Use a separate mount and underlying PVC for 3rd-party libs needed, e.g. JDBC-driver JARs or P8-API-JARs. | `false` |
| `agent.externalDependenciesVolume.existingClaim` | Persistence for the 'externalDependenciesVolume'. Agents use Kubernetes Deployments which do not support automated provisioning of volumes through 'volumeClaimTemplates'. External dependencies are e.g. P8 API JARs or JDBC driver JARs. | `""` |
| `agent.externalDependenciesVolume.storageClass` |  | `""` |
| `agent.externalDependenciesVolume.accessModes` |  | `["ReadWriteOnce"]` |
| `agent.externalDependenciesVolume.size` | Size depends on the files needed. | `"1Gi"` |
| `agent.externalDependenciesVolume.annotations` |  | `{}` |
| `agent.volumeClaims`| Additional PVC claims to mount under `/opt/sm/external-volumes/<claim-name>`. Each entry requires `name` and `claimName`. Use this for e.g. CPE or ICN logfile monitoring. | `[]`                                                                                                                                                                                     |


# 5. Configuration Examples

For values.yaml examples in one piece see section [8. Examples](#8-examples).

## 5.1 One volume approach

__NOTE:__ This approach is mutually exclusive with the seperate volumes settings described in the next section. The chart's
inbuild validation will throw an error if both are mixed in one component (Server, Agent). So you can use this approach for
e.g. the server and the other for the agent and vice versa. But we recommend to use one of them for both components. That will
allow you to reuse the agent sync files for both components from the same PVC.

If do not want to define at least three separate volumes for the H2 database, dependency injection and agent sync files,
you can instead use one single Persistent Volume Claim (PVC).

1. Create a PVC with at least 20Gi space. If you want to use the H2 database for the server, the performance of that PVC
should be quick enough for high file I/O.
2. Create the following directory structure in that PVC. This is step important and is not optional!
```
/--.
   |
   +-database/  # Create this directory even if the server does not use the H2 database!
   |
   +-deploy/
   |
   +-agents/sync/
```
3. Add the following to your `values.yaml` for the installation. Replace `<name-of-the-pvc>` by the name of the newly created PVC.
```
[...]
server:
[...]
  singleVolumePersistence:
    enabled: true
    existingClaim: <name-of-the-pvc>
[...]
agent:
[...]
  singleVolumePersistence:
...
## 5.2. SM Server Persistence

### 5.2.1. Via H2 Database on a Persistent Volume
...
### 5.2.2. Via External Database Services
...
## 5.3. Dependency Injection (JDBC drivers, P8 API JARs, etc.)
...
* Set `server.externalDependenciesVolume.enabled` to `true`
* Set `server.externalDependenciesVolume.claimName` to `sm-shared-pvc`

For the SM Agent:

* Set `agent.externalDependenciesVolume.enabled` to `true`
* Set `agent.externalDependenciesVolume.claimName` to `sm-shared-pvc`
...
## 5.4. Restricted Pod Permissions
...
### 5.4.1. Option A: RBAC Resources Created by the Chart
...
### 5.4.2. Option B: RBAC Resources Created by the User
...
### 5.4.3. Example of a Minimum Policy
...
## 5.5. Provide Root Certificates to SM Server
...
__IMPORTANT:__ The methods described in the following can not be used both at the same time for the
same component (Server, Agent). The chart's inbuild validation will throw an error if both are mixed in one
component (Server, Agent). So you can use one method in e.g. the server and the other in the agent section
and vice versa. But we recommend to use one of them for both components.

### 5.5.1 Replace cacerts of the JRE
...
### 5.5.2 Provide Certificates as Secrets
...
### 5.5.1. Provide Certificate as Secret
...
### 5.5.2. Refer to Certificate Secret in Chart Configuration
...
## 5.6. TLS protocols and ciphers for web access

You can overwrite the default web server configuration settings for the allowed or forbidden TLS protocol versions and the
allowed or forbidden cipher suites. It is not possible to have different settings for server and agent if enabled in their
sections. But you can enable the settings for only one component, e.g. the server and not the other. The other component will
then use the inbuild defaults.

Add the following section to your `values.yaml` file right after the last `global` entry. The value shown here are only examples.
```
jetty:
  tls:
    includeProtocols:
    ...
    excludeProtocols:
      ...
      - "TLSv1.0"
    includeCipherSuites:
      - "TLS_AES_256_GCM_SHA378, TLS..."  # Here you have to list **all** allowed cipher suites!
    excludeCipherSuites:
      - ".*MD5.*"
      - ".*RC4.*"
```

If you then want to use these values for the server and the agent, add the following to your `values.yaml`:
```
[...]
server:
[...]
...
The following shows a more elaborate values.yaml. It shows the two different ways to import certificates
and also shows how to mount volumes for agent-sync-file, CPE logfiles, and 3rd-party libs e.g. for
JDBC-drivers or P8-API-JARs. The examples uses the recommende way for these mounts by using a single global
volume. But is also lists the disabled settings for separate volumes.

```
global:
  imagePullSecrets:        # Optional; default is 'pull-secret-to-be-used'.
    - name: dummy-secret   # Replace it by your own secret!
...
server:
  ...
  paxWebSecurity:
    enabled: true
  # ------------------------------------------------------------------------ #
  #  'singleVolumePersistence' and separate volumes are mutually exclusive!  #
  # ------------------------------------------------------------------------ #
  singleVolumePersistence:
    enabled: true
    existingClaim: sm-global-volume
  persistence:
    enabled: false
...
agent:
  # You can use node selectors, affinity and tolerances to control the
  # nodes to be used or not to be used in the deployment process.
  affinity:
    nodeAffinity:
        ...
        nodeSelectorTerms:
          ...
          - key: topology.kubernetes.io/zone
            ...
            values:
              ...
              - antarctica-west1
  image:
    repository: company-repo.example.com:5000/sm/esmagent  # Required.
  # ------------------------------------------------------------------------------------- #
  #  'singleVolumePersistence' and 'externalDependencies Volume' are mutually exclusive!  #
  # ------------------------------------------------------------------------------------- #
  singleVolumePersistence:
    enabled: true
    existingClaim: sm-global-volume
