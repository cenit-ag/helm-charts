...
| `jetty.tls`                                                   | Optional section to explicitly override the defaults for the Jetty web server in server and agent, if enable there by `server.paxWebSecurity.enabled` and `agent.paxWebSecurity.enabled`.                                                                                            |                                                                                                                                                                                          |
...
| `jetty.tls.includeCipherSuites`                               | Optional list of allowed cipher suites. Values must follow the pattern used by Jetty and the JVM; e.g. `TLS_AES_256_GCM_SHA374`.                                                                                                                                                     | `""`                                                                                                                                                                                     |
| `jetty.tls.excludeCipherSuites`                               | Optional list of forbidden cipher suites. Values must follow the pattern used by Jetty and the JVM; e.g. `TLS_AES_256_GCM_SHA374`.                                                                                                                                                   | `".*MD5.*`,`.*RC4.*`                                                                                                                                                                     |
...
__IMPORTANT:__ The methods described in the following should never be used both at the same time for the same component (Server, Agent). It is non-deterministic, which one will win.
...
### 5.4.2 Provide Certificates as Secrets
