...
* For versions below 5.5.7.0-000 it is required to create the appropriate RBAC resources, in case the PodSecurityPolicy Admission Plugin is active for the Kubernetes cluster at hand (see more [details](#restricted-pod-permissions)). On OpenShift it is additionally required to run the containers in privileged mode for SM version lower than 5.5.7.0-000 (see more [details](#installation-on-openshift))
* For SM version 5.5.7.0-000 and above no specific RBAC resources have to be created.
...
```
docker tag smserver:5.5.7.0-000 de.icr.io/<your-namespace>/smserver:5.5.7.0-000
docker tag smagent:5.5.7.0-000 de.icr.io/<your-namespace>/smagent:5.5.7.0-000
```
...
```
docker push de.icr.io/cenit/smserver:5.5.7.0-000
docker push de.icr.io/cenit/smagent:5.5.7.0-000
```
...
For versions below 5.5.7.0-000  the SM container images partially still require root access during execution. As privileged execution of containers in disabled by default on OpenShift clusters, we need to enable this using a Service Account.
...
Run the build:
```
docker build -t smserver:5.5.7.0-000-jdbc .
```
